...
//
// Generate Markdown report from K6 summary data
// QXIP BV 2023
//
...
import mdtemplate from './markdown.ejs'
...
// Main function should be imported and wrapped with the function handleSummary
//
export function markdownReport(data, opts = {}) {
  // Default options
  if (!opts.title) {
    opts.title = new Date().toISOString().slice(0, 16).replace('T', ' ')
  }
  ...
  const markdown = ejs.render(mdtemplate, {
    data,
    title: opts.title,
    standardMetrics,
    otherMetrics,
  ...
  // Return Markdown string needs wrapping in a handleSummary result object
  // See https://k6.io/docs/results-visualization/end-of-test-summary#handlesummary-callback
  return markdown
}
